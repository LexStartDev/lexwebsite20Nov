...
myApp.controller('myCtrl', ['$rootScope', '$scope', '$http', 'fileUpload', '$window', function ($rootScope, $scope, $http, fileUpload, $window) {
	...
	$scope.service_list = [
		...
		{
			name: 'Lexgyaan Workshop 2 - Starting Up'
		},
		{
			name: 'Lexgyaan Workshop 3 - ESOPs'
		},
		{
			name: 'Lexgyaan Workshop 4 - Fundraising'
		},
		{
			name: 'Others'
		}
	...
	$scope.services = [
		'File Annual Returns', 'File DIR 3 KYC', 'Corporate Compliance', 'ESOP', 'Fundraising Documention', 'Tradmark Registration', 'Lexgyaan Workshop 2 - Starting Up', 'Lexgyaan Workshop 3 - ESOPs', 'Lexgyaan Workshop 4 - Fundraising', 'Others' 
	];
  ...
  if (valid) {
	...
	if (phoneno.test(ph_no)){
		// alert("valid");
		// return valid;
		document.getElementById('invalidContact').style.display = "none";

	  angular.forEach($scope.service_selected, function (selected, service) {
		if (selected) {
			console.log(service);
			// alert("You have selected following options " service);	
		}
	});
	$scope.interest_list = [];
	...
	//	$http.post('http://localhost:3000/api/v2/register', request).then(function (response) {
		...
		document.getElementById("directorKycForm").reset();
